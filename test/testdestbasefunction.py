...
from modules import destbasefunction

# 示例用法
if __name__ == "__main__":
    # 创建证据DAG实例
    dag = destbasefunction.EvidenceDAG()
    
    # 添加节点和证据信息
    dag.add_node('E1', frame={'a', 'b', 'c'}, reliability=0.8, node_type=destbasefunction.NodeType.AND)
    ...
    dag.add_node('A', frame={'a1', 'a2'}, reliability=1, node_type=destbasefunction.NodeType.AND)
    dag.nodes['A'].set_bpa({
        frozenset({'a1'}): 0.3,
        frozenset({'a2'}): 0.5
    })
    
    dag.add_node('E2', frame={'a', 'b', 'c'}, reliability=0.6, node_type=destbasefunction.NodeType.AND)
    ...
    dag.add_node('E3', frame={'a', 'b'}, reliability=0.9, node_type=destbasefunction.NodeType.AND)
    dag.nodes['E3'].set_bpa({
        frozenset({'a'}): 0.6,
        frozenset({'b'}): 0.4
    })
    
    dag.add_node('E4', frame={'a', 'b'}, reliability=0.5, node_type=destbasefunction.NodeType.AND)
    dag.nodes['E4'].set_bpa({
        frozenset({'a'}): 0.3,
        frozenset({'b'}): 0.7
    })
    
    dag.add_node('InterMedia', frame={'a', 'b'}, reliability=1, node_type=destbasefunction.NodeType.OR)
    # dag.nodes['InterMedia'].set_bpa({
    #     frozenset({'a'}): 0.31,
    #     frozenset({'b'}): 0.69
    # })

    dag.add_node('N', frame={'n'}, reliability=1, node_type=destbasefunction.NodeType.AND)
    dag.nodes['N'].set_bpa({
        frozenset({'n'}): 0.7
    })
    
    dag.add_node('E5', frame={'a', 'b'}, reliability=0.7, node_type=destbasefunction.NodeType.AND)
    ...
    # 创建FUSION节点
    dag.add_node('H', frame={'h1', 'h2', 'h3'}, reliability=1, node_type=destbasefunction.NodeType.FUSION)
    ...
    for node_name in dag.get_nodes():
        result = dag.calculate_node_reliability(node_name)
        print(f"节点 {node_name} 的可靠性:{result.reliability:.3f} ,确定性：{result.certification:.3f}")
    # 拓扑排序
    print("\n拓扑排序结果:")
...
# ========== 新增：测试 EvidenceDAG 类的置信度、似然度和SPDF方法 ==========
    ...
    for node_name in test_nodes:
        if node_name in dag.nodes:
            ...
            try:
                ...
                print(f"节点 {node_name}: 可靠性={result.reliability:.3f}, 置信度={p_belief:.3f}, 似然度={p_plausibility:.3f}, SPDF={p_spdf:.3f}")
            except Exception as e:
                print(f"节点 {node_name} 计算失败: {e}")
    ...
    result = dag.calculate_node_reliability('A')
    print(f"节点 'A': 可靠性={result.reliability:.3f}, 置信度={p_belief:.3f}, 似然度={p_plausibility:.3f}, SPDF={p_spdf:.3f}")
    p_belief = dag.get_propagated_belief('N')
    p_plausibility =dag.get_propagated_plausibility('N')
    p_spdf = dag.get_propagated_spdf('N')
    result = dag.calculate_node_reliability('N')
    print(f"节点 'N': 可靠性={result.reliability:.3f}, 置信度={p_belief:.3f}, 似然度={p_plausibility:.3f}, SPDF={p_spdf:.3f}")
    p_belief = dag.get_propagated_belief('H')
    p_plausibility =dag.get_propagated_plausibility('H')
    p_spdf = dag.get_propagated_spdf('H')
    result = dag.calculate_node_reliability('H')
    print(f"节点 'H': 可靠性={result.reliability:.3f}, 置信度={p_belief:.3f}, 似然度={p_plausibility:.3f}, SPDF={p_spdf:.3f}")

    p_belief = dag.get_propagated_belief('H')
    p_plausibility =dag.get_propagated_plausibility('H')
    p_spdf = dag.get_propagated_spdf('H')
    result = dag.calculate_node_reliability('H')
    print(f"节点 'H': 可靠性={result.reliability:.3f},全集的 置信度={p_belief:.3f}, 似然度={p_plausibility:.3f}, SPDF={p_spdf:.3f}")

    # 创建证据DAG实例
    Evidence1 = destbasefunction.EvidenceNode('颜色分类1')
    ...
    Evidence2 = destbasefunction.EvidenceNode('颜色分类2')
    ...
    Evidence3 = destbasefunction.EvidenceNode('颜色分类3')
    ...
    Evidence4 = destbasefunction.EvidenceNode('颜色分类4')
    ...
    combine_bpa = destbasefunction.EvidenceNode.merge_evidence_dempster(Evidence3.bpa,Evidence4.bpa,Evidence3.frame)
    print(combine_bpa)
        # 传播证据
    testnodename = 'H'
    results = dag.propagate_certification(testnodename) 
    print(f"\n 针对节点{testnodename} 的类概率函数={results:.3f}")